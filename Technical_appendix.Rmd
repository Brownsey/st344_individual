...
```{r}
my_model <- glm(count ~ 0 + week_num + season_median + appt_mode , family = "poisson", data = appt_by_mode)
poisson_data <- appt_by_mode %>%
  ...
  geom_point(aes(x = week_num, y = poisson_estimate), colour = "blue")
poisson_data

summary(my_model)
#+ offset(log_registered_patients)
```
