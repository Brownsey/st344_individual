...
model2 <- glm(count ~ 0  + week_num + season_median, family = "poisson", data = appt_by_mode %>%
              filter(appt_mode == "Face-to-Face"))


model3 <-  glm(count ~ 0  + week_num, family = "poisson", data = appt_by_mode %>%
              filter(appt_mode == "Telephone"))
model4 <- glm(count ~ 0  + week_num + season_median, family = "poisson", data = appt_by_mode %>%
              filter(appt_mode == "Telephone"))
model3 %>% summary()
model4 %>% summary()
# summary(glm(count ~ 0  + week_num + season_median, family = "poisson", data = appt_by_mode %>%
#               filter(appt_mode == "Home Visit")))

glm(count ~ season_median + week_num, family = "poisson", data = appt_by_mode) %>%
  summary()
a <- glm(count ~ season_median + week_num, family = "poisson", data = appt_by_mode) %>% summary()
sum((residuals(a))^2)

typeof(appt_by_mode$season_median)
model1 %>% summary()
model2 %>% summary()
#No idea why no pr > Chisq??? No idea what this is doing :/
anova(glm(count ~ 0 + week_num , family = "poisson", data = appt_by_mode %>%
              filter(appt_mode == "Face-to-Face")),
      glm(count ~0 +  week_num +season_median, family = "poisson", data = appt_by_mode %>%
        filter(appt_mode == "Face-to-Face")))
#Basically shows model 2 with season so much better
BIC(model1)
BIC(model2)
```
