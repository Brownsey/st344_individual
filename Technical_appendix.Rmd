...
```
## Lab work
This section of the technical appendix will cover the code produced as part of lab 6, which was used to guide the starting point for the individual project work in general in terms of datasets and provided a good base for exploratory data analysis in particuar for pointing out the effect of different days.


```{r lab_functions}
#function to lower case column names as in general it is better 
#and easier to refer to everything in the lower case
...
There has also been no subsetting based on hcp_type -> as all appointments is theoretically workload (if admin went through the room would still an increased workload for example)
...
p2
#Autumn shift not taken account of in the year 1 vs year 2 comparison and could yeild a no change when a ctual change...
p3 <- appt_by_mode %>%
...
#Could include but takes up too much room so probably just include telephone
pic <- grid.arrange(p1,p2,p3,p4,nrow = 2)

# Telephone Pic for report:
p4 + theme(axis.text=element_text(size=12),
...
#Changing axis ready for submission
...
##Number of DNAs by average number of days + 1
...
```
### Extra plots
Looking at some other plotting methods to show interest

```{r}
#Barchart for season values ~ look at median - to account for bit drop offs or increases.
#Needs to be done for a specific appointment type, in this case: 
#F2F as that was only one with significant difference and good to showcase that.
bar_df <- stat_data %>%
  filter(appt_mode == "Face-to-Face") %>%
  group_by(season_median) %>%
...
bar_df %>%
  ...
  theme_bw() +
  theme(legend.position = "none") +
  labs(title ="Median Weekly Face-to-Face Appointment Count by Season",
       y = "Median Weekly Face-to-Face Appointment Count",
       x = "Season")
...
#3d barchart
library(plotrix)
#Step_1 work out percentage of appointments which come under each type

pie_values <-stat_data %>%
  group_by(appt_mode) %>%
  summarise(total = sum(count)) %>%
  ungroup() %>%
  mutate(percentage = (100 * total)/(2658098 +40910 +334646)) %>%
  select(percentage) %>%
  as_vector()


 pielabels<- c("Face-to-Face","Home Visit","Telephone")
 # grab the radial positions of the labels
 lp<-pie3D(pie_values,radius=0.9,labels=pielabels,explode=0.4,main="3D PIE OPINIONS")
